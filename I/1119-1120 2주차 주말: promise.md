...
### 4.1 resolve된 promise를 then으로 확인하는 과정 이해하기
...
```
멤버 변수를 초기화하고 0초 후에 callback 메서드에 MyPromise 클래스에서 구현한 this.resolve()와 this.reject() 메서드를 인자로 넘겨주면서 callback을 실행하고 있습니다.

콘솔에 출력된 부분을 보면 then()이 먼저 실행된 다음에 constructor의 callback 함수가 실행됩니다.

[setTimeout](https://developer.mozilla.org/ko/docs/Web/API/setTimeout)의 2번째 파라미터에 delay할 ms를 지정할 수 있는데 delay paramter를 생략하거나 0으로 설정하면 다음 이벤트 사이클에 실행하도록 합니다.

0초 후라는 트릭을 사용해 비동기를 흉내내어 then으로 먼저 함수를 등록한 다음에 callback 함수를 실행하도록 구현했습니다.

then()에서는 this.callbackWhenResolved에 callback 함수를 등록하는 일을 합니다. 함수가 등록되어 있어야 resolve를 호출했을 때 this.callbackWhenResolved를 사용할 수 있기 때문에 callback 함수 즉, durationInMillis로 판단해 resolve 또는 reject를 실행하는 함수를 다음 이벤트 사이클 때 실행되도록 하고 then()이 먼저 실행되록 하기 위해 `setTimeout(code, 0);`을 사용했습니다.

사실 현재 코드에서는 delay 함수에서 받은 promise에 then이 아닌 try ~ catch 문으로 확인하고 있고 delay 함수 내에서 resolve() 호출 시 아무 인자를 넘겨주고 있지 않아 대체 어떤 함수가 등록된다는 말인지 이해가 안 갈 수 있습니다. 만약 someMethod 함수를 아래와 같이 변경해본다면 
...
```
then이 실행될 때 this.callbackWhenResolved에 `() => { console.log("then test:"); }`가 등록되는 것을 볼 수 있습니다.
...
생성된 MyPromise 객체에서 then()이 먼저 실행돼 callbackWhenResolved에 함수가 잘 등록된 상태에서 resolve 함수를 호출한 것입니다.
