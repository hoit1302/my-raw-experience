...
아주 간단한 Promise와 간단한 Promise 이 두 가지 버전을 순서대로 진행합니다.
...
**start()는 MyPromise의 객체를 리턴하여 MyPromise의 then() 메서드를 호출하는데 then() 메서드의 파라미터로 넘겨준 'javascript'를 출력하여 모든 코드 실행이 끝납니다.**
...
자세한 설명을 시작하겠습니다.
JS에서는 함수는 굉장히 특별하게 취급받습니다. 함수 자체를 다른 함수의 인자 값으로 사용할 수 있고 함수 자체를 리턴할 수도 있습니다. JAVA에서는 불가능한 일입니다.
...
두 코드 모두 text를 인자로 받아서 콘솔에 출력하는 printText 함수를 선언했다는 뜻입니다.
두 선언 방식의 차이점에 대해 간략히 설명하면, const 형태의 arrow function은 hoisting을 지원하지 않기 때문에 호출하려는 함수 위에 선언되어야 한다는 것입니다.
...
다음 줄에서 바로 constructorCallback을 호출하는데, 이 함수는 위에 보이다시피 myfunc이라는 인자를 가지고 있습니다.

인자인 myfunc에는 또 아래의 함수 자체가 전달되고 바로 다음 줄에서 myfunc('javascript') 을 호출합니다. 
...
즉, 위 함수에서 'javascript'가 value라는 인자로 들어가는 것입니다.

this 키워드를 사용해서 생성된 객체의 멤버 변수로 사용할 수 있습니다. 그래서 MyPromise의 멤버 변수인 this.value에 value라는 인자로 전달된 'javascript' 문자열을 저장하게 되었습니다. 🎉

드디어 start()의 MyPromise 생성을 마치고 MyPromise 객체를 리턴했습니다.

then 부분을 살펴보겠습니다. then 부분은 쉽습니다.

앞선 과정으로 this.value 라는 멤버 변수에 'javascript'가 저장되어 있습니다. '.' (dot) 으로 멤버 변수에 접근하듯이 class의 메서드도 '.' 으로 접근할 수 있습니다.

MyPromise에 선언되어 있는 then은 thenCallback라는 인자를 가지고 있습니다.
...
위 코드에서는 thenCallback에 아래의 함수가 전달됩니다.
...
callback 함수를 받고 다음 줄에서 바로 thenCallback을 실행하는데 인자가 'javascript'가 저장되어 있는 this.value 이므로 result에 'javascript'가 인자로 전달되어 'javascript'가 출력됩니다.

지금까지는 사실 Promise보다는 콜백 함수를 깊게 다뤘다고 말하는 것이 맞습니다.
Promise의 핵심이라고 할 수 있는 비동기의 개념이 들어가지 않았기 때문인데요, 덜 간단한 promise를 구현해봅시다.

## 2. 비동기: 개념, promise, async & await 사용법 이해하기
...
async, await 키워드가 눈에 띕니다.
**await는 비동기 함수가 실행되고 결과를 받을 때까지 기다리라는 뜻**입니다.
주의할 점은 **await 키워드는 Promise를 반환하는 비동기 함수 안에서만 작성할 수 있다**는 것입니다. 그래서 () => {} 함수 앞에 async 키워드를 붙여서 "이 함수는 비동기 함수다" 라고 알려준 것입니다. 그리고 **async 함수는 promise를 반환합니다.**

async, await는 왜 사용할까요? 
async, await는 Promise를 사용하는 비동기 처리를 더 쉽게 사용하도록 도와줍니다.
아직 Promise도 잘 모르기 때문에 Promise가 어떻게 사용되는 것인지, 비동기란 무엇인지 알아본 후에 질문의 답을 이해해봅시다.
...
someMethod()는 Promise를 반환하는 비동기 메서드입니다. **Promise는 비동기 작업이 성공적으로 완료되면 .then() 로 return 값을 전달받아 처리할 수 있습니다.** someMethod() 에서는 3을 리턴합니다. // * 여기에서 then() 메서드가 3을 result 인자로 받아 콘솔에 3을 출력하고 다시 someMethod()을 리턴합니다.
...
만약 두 번째 someMethod()가 오류를 전달했다고 가정하면 promise를 성공해야 실행될 수 있는 then으로는 처리할 수 없기 때문에 콘솔에 3이 1번만 출력되고 // ** 이후로는 전부 실행이 되지 않습니다.
...
동기와 비동기 어떤 차이점이 있을까요?

동기는 요청 후 결과가 반환될 때까지 대기하고 있다가 결과를 받은 후 다음 요청을 수행합니다.
하지만 비동기는 요청 후 결과를 마냥 기다리지 않고 바로 다음 작업을 수행합니다. 
...
그러면 비동기 작업 결과를 어떻게 활용하나요?

비동기 작업을 요청했다면 반환된 내용을 사용해야 하는데, 비동기 작업이 끝나 결과가 반환되었을 때를 다루기 위해서 앞서 설명한 Promise가 등장한 것입니다. 더 나아가 then, function, return 문을 감추어 가독성을 높이기 위해 async, await가 등장했습니다.

### async, await: promise 비동기 개선
...
## 3. 비동기 개념이 추가된 Promise 코드 구현하기

요구사항으로 promise 개념을 정리해보았습니다.
...
delay 함수에서는 MyPromise를 생성할 때 콜백 함수를 넘겨주고 있습니다.

```js
class MyPromise {
  constructor(callback) {
```
constructor의 callback은 아래의 함수를 뜻합니다.
...
```
MyPromise의 생성자 내용을 이어서 보면 멤버 변수를 초기화하고 0초 후에 callback 메서드에 MyPromise 클래스에서 구현한 this.resolve()와 this.reject() 메서드를 자체를 인자로 넘겨주면서 callback을 실행하고 있습니다.

눈에 띄는 부분은 콘솔에 출력된 부분을 보면 then()이 먼저 실행된 다음에 constructor의 callback 함수가 실행됩니다.

then()에서는 this.callbackWhenResolved에 callback 함수를 등록하는 일을 합니다. 함수가 등록되어 있어야 resolve를 호출했을 때 사용할 수 있으니 당연하게 받아들이시면 됩니다.

0초 후라는 트릭을 사용해 비동기를 흉내내어 then으로 먼저 함수를 등록한 다음에 callback 함수를 실행하도록 구현했습니다.
...
생성된 MyPromise 객체에서 then()이 먼저 실행돼 callbackWhenResolved가 잘 등록된 상태의 resolve 함수를 호출한 것입니다.
...
wrongMethod()를 호출해서 이번에는 value가 전달되는 과정을 한 번 살펴보겠습니다.
...
return value: -5
```
-500을 durationInMillis 인자로 전달됩니다.
...
이후에 생성자 안의 callback 함수에 MyPromise의 resolve()와 reject() 메서드를 인자로 넘겨줍니다. 이 때, resolve()의 this.callbackWhenResolved 은 함수가 지정되어 있는 상태입니다. then()이 이전에 호출되었으니까요. reject()도 마찬가지입니다.

callback 함수에서 -500은 reject(new Error(`Duration ~`)); 를 호출합니다. reject 함수에 생성된 error 객체를 전달합니다.
reject 함수는 아래처럼 정의된 함수를 뜻하고, 인자를 value로 받아서 다시 인자를 넘겨주고 있습니다.
...
현재, callbackWhenResolved 뿐만 아니라 callbackWhenCaught 이 멤버 변수도 then에서 함수를 등록하고 있기 때문입니다. promise 표준에서도 then 1개만 존재한다고 합니다.
...
이런 결과를 얻을 수 있습니다.
catch 함수의 callback으로 아래 함수가 전달되어 callbackWhenCaught 멤버 변수에 등록됩니다.
