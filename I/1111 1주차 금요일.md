...
보통 원오원이라고 말한다. 30분하는데 오늘은 1시간을 했다.
...
start() {
  document.getElementById('save_png').addEventListener('click', () => {
    saveScreenshotWithType('image/png');
  });
}
```
...
git을 몰라도 git을 사용할 수 있도록 툴을 만드셨다.
...
매번 이런 일을 겪었다. 뭔가 삘받아서 더 적어보고 싶다.
...
결론은 구구절절 설명했던 부분에 대해서 굉장히 부드럽게 진행된다. 더 자세히는 force 명령어 없이 진행된다.

**작업을 신나게 하다가 2번 명령어를 실행시키면 base만 바뀌는 느낌이다.**

```
만약 내가 코드에서 save를 create로 바꿔서 vsc 왼쪽에서 노란색으로 변경되었다고 표시되어 있다고 하자.
그런데 다른 사람이 나와 똑같이 변경했고 그 코드가 main으로 merge 되었을 때 2번 명령어를 실행하면 
create로 나는 변경했다고 생각한 부분이 아무 변경 사항이 없다고 노란색 표시가 사라진다.
```
